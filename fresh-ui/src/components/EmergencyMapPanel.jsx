...
export default function EmergencyMapPanel({ myLocation, services, activeResponder, movingResponder, routePath, onRequestHelp }) {
  const pathCoordinates = movingResponder
    ? [[movingResponder.lat, movingResponder.lng], myLocation]
    : routePath;
  ...
  return (
    ...
    >
      ...
      <div className="h-[520px] w-full rounded-[2rem] overflow-hidden">
        <MapContainer center={myLocation} zoom={13} zoomControl={false} className="h-full w-full">
          <TileLayer
            url="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png"
            attribution='&copy; <a href="https://carto.com/">CARTO</a>'
          />
          <Marker position={myLocation} icon={L.divIcon({
            className: 'bg-transparent',
          ...
          {movingResponder && (
            <Marker
              position={[movingResponder.lat, movingResponder.lng]}
              icon={L.divIcon({
                className: 'bg-transparent',
