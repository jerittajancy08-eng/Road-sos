...
export default function LiveMapSection({ location, services, routePath, movingResponder }) {
  const center = safePosition(location);
  const movingPosition = movingResponder ? positionFromLatLng(movingResponder.lat, movingResponder.lng) : null;
  const mapPath = validPositions(movingPosition ? [movingPosition, center] : routePath || []);
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="glass-card border border-white/10 overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
      ...
      <div className="h-[420px] sm:h-[480px] w-full bg-slate-950">
        {!isValidPosition(center) ? <LoadingMap /> : <MapContainer center={center} zoom={13} zoomControl={false} className="h-full w-full">
          ...
          <Marker position={center} icon={L.divIcon({ className: 'bg-transparent', html: '<div class="h-4 w-4 rounded-full border border-white/40 bg-cyan-400 shadow-[0_0_18px_rgba(56,189,248,0.35)]"></div>', iconSize: [16, 16], iconAnchor: [8, 8] })}>
          ...
          {mapPath.length > 1 && <Polyline positions={mapPath} pathOptions={{ color: "#38bdf8", weight: 5, opacity: 0.8 }} />}
        </MapContainer>}
      </div>
    </motion.div>
  );
